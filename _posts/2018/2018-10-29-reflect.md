...
&nbsp;&nbsp;以下为 Field里面获取属性值得实现,先通过override做校验，如果没有重载该权限，则校验访问权限
```
...
&nbsp;&nbsp;以上代码可以发现，通过工厂模式根据field属性类型以及是否静态获取，为什么会有这样的划分呢
&nbsp;&nbsp;首先，jdk是通过UNSAFE类对 堆内存中对象的属性进行直接的读取和写入，要读取和写入首先需要确定属性存的位置，也就是相对对象起始位置的偏移量，针对静态属性，是针对类的不是每个对象实例一份，所以静态属性的偏移量需要单独获取，起始通过该偏移量可以大致推断出一个实例内每个属性在堆内存的相对位置，以及分别占用多大的空间，有了位置信息，我们还需要这个字段的类型，以方便执行器知道读几个字节的数据，并且如何进行解析，目前提供了8大基础类型（char vs Charector）和数组和普通引用类型。
&nbsp;&nbsp;以下为UnSafe类的部分代码
...
&nbsp;&nbsp;经过以上优化，其实反射的效率并不慢，在某些情况下可能达到和直接调用基本相同的效率，但是还是会有性能上的开销，主要在以下方面

1. 1.Class.forName();会调用本地方法，我们用到的method和field都会在此时加载进来，虽然会进行缓存，但是本地方法免不了有JAVA到C+=在到JAVA得转换开销
2. 2.class.getMethod()，会遍历该class所有的公用方法，如果没匹配到还会遍历父类的所有方法，并且getMethods()方法会返回结果的一份拷贝，所以该操作不仅消耗CPU还消耗堆内存，在热点代码中应该尽量避免，或者进行缓存
3. 3.invoke参数是一个object数组，而object数组不支持java基础类型，而自动装箱也是很耗时的

### 反射的运用
- spring doc

&nbsp;&nbsp;spring加载bean的流程基本都用到了反射机制
1. 1.获取类的实例 通过构造方法getInstance（静态变量初始化，属性赋值，构造方法）
2. 2.如果实现了BeanNameAware接口，则用反射注入bean赋值给属性
3. 3.如果实现了BeanFactoryAware接口，则设置 beanFactory
4. 4.如果实现了ApplicationContextAware，则设置ApplicationContext
5. 5.调用BeanPostProcesser的预先初始化方法
6. 6.如果实现了InitializingBean,调用AfterPropertySet方法
7. 7.调用定制的 init-method()方法  对应的直接 @PostConstruct
8. 8.调用BeanPostProcesser的后置初始化完毕的方法
...
&nbsp;&nbsp;我们分析下这个方法的实现，首先生成的代理对象，需要实现参数里面声明的所有接口，接口的实现应给委托给InvocationHandler进行处理，所以所生成的代理类里面必须能够获取到InvocationHandler 在我们无法知道代理类的具体类型的时候，我们可以通过构造方法将InvocationHandler传给代理类的实例
&nbsp;&nbsp;所以 总的来说生成代理对象需要两步
1. 1.获取代理类的class对象
2. 2.通过class对象获取构造方法，通过反射生成代理类的实例，并将InvocationHandler传人
```
@CallerSensitive
...
&nbsp;&nbsp;下面我们在看下 getProxyClass0 如何获取代理类的class对象，这里idk通过WeakCache来缓存已经生成的class对象，因为生成该class通过字节码生成还是很耗时，同时为了解决之前由于动态代理使用太多导致内存不足，所以这里通过弱引用WeakReference来缓存所生成的代理对象，当发生GC的时候如果该class对象没有其他的强引用将会被直接回收
...
```
上面的流程可以简单归纳为
1. 1.增加hashcode,equals,toString方法
2. 2.增加所有接口中声明的未实现方法
3. 3.增加一个方法参数为java/lang/reflect/InvocationHandler的构造方法
4. .其他静态初始化数据

&nbsp;&nbsp;了解了jdk动态代理的实现，我们来看下下面这个经典的面试问题
一个类里面 两个方法A和方法B   方法B上有加注解做事物增强 那么A调用B为什么没有事物？  因为spring aop默认基于 jdk的动态代理，动态代理是基于 invocationHandler里面 通过 method，invoke 前后做增强，而 method.invoke 里面的参数就是被代理的对象，而被代理的对象方法B并没有做增强  如果spring aop是基于cglib字节码增强技术实现则会有事物效果
jdk的动态代理是通过，ProxyGenerate 生成一个二进制class文件，然后在通过类加载器把这个二进制文件加载进来，并且会把这个类缓存起来，目前是weak引用做的缓存，避免OOM，生成的class文件对应的类有一个构造方法，构造方法的参数就是InvocationHandler ，而 InvocationHandler的构造方法参数就是被代理的对象 cglib会默认继承自被代理的对象，如果在A方法调用this.B方法 则会调用被增强的B方法